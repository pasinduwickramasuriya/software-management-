...
export default function ExecutiveOfficerDashboard() {
  ...
  const handleDecisionSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Ticket ${decisionType === 'approved' ? 'Approved' : 'Rejected & sent back to Branch Manager'}!`);
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'pending_executive':
        return <span style={{ ...badgeStyle, bg: '#fef3c7', color: '#b45309' }}><Clock size={12} /> Pending Executive Review</span>;
